...
class Desenho:
    ...
    def listar_no_e_filhos(raiz):
        ...
        if raiz is not None:
            ...
            if raiz.esquerda!=None:
                cpf_esquerda=str(raiz.esquerda.cpf) 
            if raiz.direita!=None:
                cpf_direita=str(raiz.direita.cpf) 
            no_e_filhos.append((str(raiz.cpf), raiz.nome, cpf_esquerda, Desenho.get_cpf_filho(raiz.esquerda), cpf_direita, Desenho.get_cpf_filho(raiz.direita)))
            no_e_filhos.extend(Desenho.listar_no_e_filhos(raiz.esquerda))
            no_e_filhos.extend(Desenho.listar_no_e_filhos(raiz.direita))
        return no_e_filhos

    @staticmethod
    def get_cpf_filho(filho):
        return filho.cpf if filho else None
    ...
    def desenhar(self, raiz):
        ...
        for i, (no, pessoa, no_esquerda, filho_esquerda, no_direita, filho_direita) in enumerate(no_e_filhos):
            self.dot.node(str(no), pessoa)
        ...
        for i, (no, pessoa, no_esquerda, filho_esquerda, no_direita, filho_direita) in enumerate(no_e_filhos):
            if filho_esquerda is not None or filho_direita is not None:
                if filho_esquerda is not None:
                    self.dot.edge(str(no), str(no_esquerda), label='esq')
                if filho_direita is not None:
                    self.dot.edge(str(no), str(no_direita), label='dir')
