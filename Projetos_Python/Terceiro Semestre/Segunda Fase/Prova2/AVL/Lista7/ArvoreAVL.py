...
class ArvoreAVL(Arvore):
    ...
    @staticmethod
    def insereBalanceado(raiz: No, novo: No):
        if raiz == None:
            raiz=novo
            return raiz
        if raiz.valor>novo.valor:
            ArvoreAVL.prova(raiz.esquerda, novo)
        else:
            ArvoreAVL.prova(raiz.direita, novo)
        if ArvoreAVL.fator_balanceamento(raiz) >1:
            if ArvoreAVL.altura(raiz.esquerda.esquerda)>ArvoreAVL.altura(raiz.esquerda.direita):
                ArvoreAVL.rotacao_dir(raiz)
            else:
                ArvoreAVL.rotacao_dupla_dir(raiz)
        else:
            if ArvoreAVL.altura(raiz.direita.esquerda)>ArvoreAVL.altura(raiz.direita.direita):
                ArvoreAVL.rotacao_dupla_esq(raiz)
            else:
                ArvoreAVL.rotacao_esq(raiz)
    ...
    @staticmethod
    def remover_rec(raiz: No, valor):
        raiz = Arvore.remover_rec(raiz, valor)
        return ArvoreAVL.balancear(raiz)