...
class Arvore:
    ...
    def numFolhas (raiz: No) -> int:
        ...
        if raiz.esquerda == None and raiz.direita == None:
            return 1
        return Arvore.numFolhas(raiz.esquerda) + Arvore.numFolhas(raiz.direita)   

    """
    função recursiva que apaga todas as folhas de uma árvore que tenham a chave
    igual a um valor dado
    """ 

    def removeFolha (raiz: No, x: int):
        if raiz == None:
            return 
        if raiz.esquerda == None and raiz.direita == None and raiz.valor == x:
            raiz=None
            return
        esq = Arvore.removeFolha(raiz.esquerda, x)  
        dir = Arvore.removeFolha(raiz.direita, x)     
...
Arvore.removeFolha(novo_2, 7)

print("Árvore:")
Arvore.mostraArvore(novo_2, 2)
