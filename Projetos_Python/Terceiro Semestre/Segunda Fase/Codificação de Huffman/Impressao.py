...
class Desenho:
    ...
    def listar_no_e_filhos(raiz):
        no_e_filhos = []
        if raiz is not None:
            no_e_filhos.append((raiz, raiz.carac, raiz.esquerda, Desenho.get_carac_filho(raiz.esquerda), raiz.direita, Desenho.get_carac_filho(raiz.direita)))
            no_e_filhos.extend(Desenho.listar_no_e_filhos(raiz.esquerda))
            no_e_filhos.extend(Desenho.listar_no_e_filhos(raiz.direita))
        return no_e_filhos

    @staticmethod
    def get_carac_filho(filho):
        return filho.carac if filho else None
    ...
    def desenhar(self, raiz):
        ...
        for i, (no, carac, no_esquerda, filho_esquerda, no_direita, filho_direita) in enumerate(no_e_filhos):
            self.dot.node(str(no), str(carac))
        ...
        for i, (no, carac, no_esquerda, filho_esquerda, no_direita, filho_direita) in enumerate(no_e_filhos):
            if filho_esquerda is not None or filho_direita is not None:
                if filho_esquerda is not None:
                    self.dot.edge(str(no), str(no_esquerda), label='0')
                if filho_direita is not None:
                    self.dot.edge(str(no), str(no_direita), label='1')
